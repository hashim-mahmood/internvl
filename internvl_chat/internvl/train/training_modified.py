...
class DataTrainingArguments:
    ...
    eval_meta_path: Optional[str] = field(
        default=None,
        metadata={'help': 'The path of the meta file for evaluation dataset.'},
    )
...
def build_datasets(
    ...
    # Use the provided meta_path or fall back to data_args.meta_path
...
def main():
    ...
    # Build evaluation dataset if a separate evaluation meta file is provided
    if data_args.eval_meta_path is not None:
        eval_dataset = build_datasets(
            ...
            normalize_type=data_args.normalize_type, min_num_frame=data_args.min_num_frame,
            max_num_frame=data_args.max_num_frame,
            meta_path=data_args.eval_meta_path,
            is_train=False
        )
    else:
        eval_dataset = None
